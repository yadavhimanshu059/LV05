...
reset_params <- function(){
  ...
  cueweighting <<- 1     # Strength of structural cue as ratio structural/morphological
...
compute_int_means <- function(results){
	...
	suppressMessages(left_join(intMeans, params))
}
...
###############################################################
# percolation with probability, perc

distortion <- function(perc){
  # perc is probability of feature percolation from distractor to target
  I_perc <- rbinom(1,1,perc)
  if(I_perc==0){
    model_4cond <- list(
    ...
    model_4cond <- list(
      target_match = list(c(1,1), c(1,0), c(1,1), c(1,0)),
      distractor_match = list(c(0,1), c(0,0), c(0,1), c(0,0)),
      Target = c("Match","Match","Mismatch","Mismatch"),
      Distractor = c("Match","Mismatch","Match","Mismatch"),
      weights = list(c(strWeight(),semWeight()))
...
create_param_matrix <- function(perc,model,parameters,iterations=1000,verbose=FALSE){
